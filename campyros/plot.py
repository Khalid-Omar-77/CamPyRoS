...
def plot_ypr(simulation_output, rocket):
    ...
    for (
        index,
        row,
    ) in simulation_output.iterrows():  # this is ugly but proper way not working
...
def plot_launch_trajectory_3d(
    ...
    pos_i_array = np.stack(
        simulation_output["pos_i"], axis=0
    )  # np.stack(Series, axis=0) will convert a panda Series into a numpy ndarray.
    pos_l_array = np.zeros(pos_i_array.shape)

    for i in range(len(pos_l_array)):
        pos_l_array[i] = pos_i2l(
            pos_i_array[i], rocket.launch_site, simulation_output["time"][i]
        )
    x_l = pos_l_array[:, 0]
    y_l = pos_l_array[:, 1]
    z_l = pos_l_array[:, 2]
    ...
    burnout_index = (
        np.abs(np.array(simulation_output["time"]) - burnout_time)
    ).argmin()
    ax.scatter(
        x_l[burnout_index],
        y_l[burnout_index],
    ...
    if show_orientation == True:
        ...
        for i in range(len(xb_i_array)):
            xb_l_array[i] = direction_i2l(
                xb_i_array[i], rocket.launch_site, simulation_output["time"][i]
            )
...
def plot_altitude_time(simulation_output, rocket):
    ...
    vel_i_array = np.stack(
        simulation_output["vel_i"], axis=0
    )  # np.stack(Series, axis=0) will convert a panda Series into a numpy ndarray
    vel_l_array = np.zeros(pos_i_array.shape)

    for i in range(len(pos_l_array)):
        vel_l_array[i] = vel_i2l(
            vel_i_array[i], rocket.launch_site, simulation_output["time"][i]
        )
    ...
    axs[1, 0].plot(
        simulation_output["time"],
        (vx_l ** 2 + vy_l ** 2 + vz_l ** 2) ** 0.5,
        "tab:green",
    )
...
def plot_aero(simulation_output, rocket):
    ...
    for i in range(len(output_dict["time"])):
        ...
        force_b, cop, q = rocket.aero_forces(
            output_dict["pos_i"][i],
            output_dict["vel_i"][i],
            b2i,
            output_dict["w_b"][i],
        ...
        damping_moment = rocket.aero_damping_moment(
            output_dict["pos_i"][i],
            output_dict["vel_i"][i],
            b2i,
            output_dict["w_b"][i],
        ...
        v_rel_wind = b2i.inv().apply(
            direction_l2i(
                (
                    ...
                    - rocket.launch_site.wind.get_wind(lat, long, alt)
                ),
                rocket.launch_site,
                output_dict["time"][i],
            )
    ...
    axs[0, 0].plot(
        output_dict["time"], force_x_b, label="X-force (body-coordinates)", color="red"
    )
    axs[0, 0].plot(
        output_dict["time"],
        force_y_b,
        label="Y-force (body-coordinates)",
        color="green",
    )
    axs[0, 0].plot(
        output_dict["time"], force_z_b, label="Z-force (body-coordinates)", color="blue"
    )
    ...
    axs[0, 1].plot(
        output_dict["time"],
        moment_x_b,
        label="X-moment (body-coordinates)",
        color="red",
    )
    axs[0, 1].plot(
        output_dict["time"],
        moment_y_b,
        label="Y-moment (body-coordinates)",
        color="green",
    )
    axs[0, 1].plot(
        output_dict["time"],
        moment_z_b,
        label="Z-moment (body-coordinates)",
        color="blue",
    )
    ...
    axs[1, 0].plot(
        output_dict["time"], np.array(q_data) / 1000, label="Dynamic pressure"
    )
    ...
    axs[1, 1].set_title(
        "Angle of attack, centre of pressure (COP), and centre of gravity (COG)"
    )
    axs[1, 1].plot(
        output_dict["time"],
        np.array(alpha_data) * 180 / np.pi,
        color="red",
    ...
    cop_axis = axs[
        1, 1
    ].twinx()  # Plot COP and COG on the same graph as the angle of attack, but use a different y-scale
...
def plot_thrust(simulation_output, rocket):
    ...
    for i in range(len(output_dict["time"])):
        thrust_b, jet_damping_moment = rocket.thrust(
            output_dict["pos_i"][i],
            output_dict["vel_i"][i],
            Rotation.from_matrix(output_dict["b2imat"][i]),
            output_dict["w_b"][i],
            output_dict["time"][i],
        )
        ...
        if output_dict["time"][i] < max(rocket.motor.motor_time_data):
            mdot = np.interp(
                output_dict["time"][i],
                rocket.motor.motor_time_data,
                rocket.motor.mdot_data,
            )
        else:
            mdot = 0
    ...
    fig, axs = plt.subplots(2, 2)
    axs[0, 0].plot(
        output_dict["time"], thrust_x_b, label="X-force (body-coordinates)", color="red"
    )
    axs[0, 0].plot(
        output_dict["time"],
        thrust_y_b,
    ...
    axs[0, 0].plot(
        output_dict["time"],
        thrust_z_b,
        label="Z-force (body-coordinates)",
        color="blue",
    )
    axs[0, 0].axvline(burnout_time, label="Burnout time", linestyle="--", color="black")
...
def plot_mass(simulation_output, rocket):
    ...
    axs[0, 1].axvline(
        burnout_time, label="Burnout time", linestyle="--", color="orange"
    )
    axs[0, 1].legend()
    axs[0, 1].grid()
...
def animate_orientation(simulation_output, frames=500):
    ...
    axs[0, 0].plot(
        np.linspace(0, np.cos(yaw[0]), 100),
        np.linspace(0, np.sin(yaw[0]), 100),
        lw=3,
        color="black",
    )
    axs[0, 1].plot(
        np.linspace(0, np.cos(pitch[0]), 100),
        np.linspace(0, np.sin(pitch[0]), 100),
        lw=3,
        color="black",
    )
    axs[1, 0].plot(
        np.linspace(0, np.cos(roll[0]), 100),
        np.linspace(0, np.sin(roll[0]), 100),
        lw=3,
        color="black",
    )
    ...
    anim1 = FuncAnimation(fig, update1, init_func=init1, frames=frames, blit=True)
    anim2 = FuncAnimation(
        fig, update2, init_func=init2, frames=frames, interval=20, blit=True
    )
    anim3 = FuncAnimation(
        fig, update3, init_func=init3, frames=frames, interval=20, blit=True
    )
    anim4 = FuncAnimation(
        fig, update4, init_func=init4, frames=frames, interval=20, blit=True
    )
...
def stats_landing(mu, cov, data=pd.DataFrame(), sigma=3):
    ...
    cov_elipse = np.matmul(
        eig_mat, np.array([eig[0][1] ** 0.5 * np.cos(t), eig[0][0] ** 0.5 * np.sin(t)])
    )

    for sig in range(1, sigma + 1):
        ax.plot(
            sig * cov_elipse[0] + mu[0],
            sig * cov_elipse[1] + mu[1],
            label="%s $\sigma$" % sig,
            linewidth=1,
        )
    ...
    ax.scatter(
        mu[0], mu[1], marker="o", s=20, color="black", label="Mean landing point"
    )
...
def stats_apogee(
    ...
    ap_eig_mat = np.array(
        [
            [ap_eig[1][0][0], ap_eig[1][1][0], ap_eig[1][2][0]],
            [ap_eig[1][0][1], ap_eig[1][1][1], ap_eig[1][2][1]],
            [ap_eig[1][0][2], ap_eig[1][1][2], ap_eig[1][2][2]],
        ]
    )
    ...
    for v in np.linspace(0, np.pi, 100):
        for u in np.linspace(0, 2 * np.pi, 200):
            d = elipse(
                u, v, ap_eig[0][0] ** 0.5, ap_eig[0][1] ** 0.5, ap_eig[0][2] ** 0.5
            )
    ...
    for sig in range(1, sigma + 1):
        ax.plot(
            apogee_mu[0] + sig * np.array(x),
            apogee_mu[1] + sig * np.array(y),
            apogee_mu[2] + sig * np.array(z),
            alpha=0.2,
        )
    ...
    if not apogee.empty:
        ax.scatter(
            apogee.x, apogee.y, apogee.alt, marker="o", s=10, color="blue", alpha=0.3
        )
    ...
    if landing_mu.shape != (0,) and landing_cov.shape != (0,):
        ...
        l_eig_mat = np.array(
            [[l_eig[1][0][0], l_eig[1][1][0]], [l_eig[1][0][1], l_eig[1][1][1]]]
        )
        landing_cov_elipse = np.matmul(
            l_eig_mat.transpose(),
            np.array([l_eig[0][0] ** 0.5 * np.cos(t), l_eig[0][1] ** 0.5 * np.sin(t)]),
        )

        for sig in range(1, sigma + 1):
            ax.plot(
                sig * landing_cov_elipse[0] + landing_mu[0],
                sig * landing_cov_elipse[1] + landing_mu[1],
                0,
                label="%s $\sigma$" % sig,
                linewidth=1,
            )
        ...
        ax.scatter(
            ...
            s=20,
            color="black",
            label="Mean landing point",
        )

        if not landing.empty:
            ax.scatter(
                landing.x, landing.y, 0, marker="o", s=10, color="blue", alpha=0.3
            )
...
def stats_alt(z, t, show_means=False, sigma=3):
    ...
    if show_means == True:
        ...
        st_dev = st_dev[1:]
        plt.plot(
            [incriments * n for n in range(0, len(mean_alts))],
            mean_alts,
            color="red",
            label="Mean",
        )
        for sig in range(1, sigma + 1):
            plt.plot(
                [incriments * n for n in range(0, len(mean_alts))],
                np.array(mean_alts) + np.array(st_dev) * sig,
                alpha=(0.9 - 0.1 * sig),
                color="orange",
                label="%s$\sigma$" % sig,
                linewidth=1,
...
def stats_trajectories(
    ...
    if (
        apogee_mu.shape != (0,)
        and apogee_cov.shape != (0,)
        and landing_mu.shape != (0,)
        and landing_cov.shape != (0,)
    ):
        ap_eig = np.linalg.eig(apogee_cov)
        ap_eig_mat = np.array(
            [
        ...
        if landing_mu.shape != (0,) and landing_cov.shape != (0,):
            ...
            landing_cov_elipse = np.matmul(
                l_eig_mat.transpose(),
                np.array(
                    [l_eig[0][0] ** 0.5 * np.cos(t), l_eig[0][1] ** 0.5 * np.sin(t)]
                ),
            )
